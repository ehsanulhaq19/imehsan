    ...
    <>
      <header className="fixed left-0 right-0 top-0 z-[100] border-b border-brand-outline-soft/20 bg-brand-bg/85 backdrop-blur-md">
        <div className="mx-auto flex max-w-content items-center justify-between gap-3 px-page-x py-4 md:gap-10 md:px-page-x-md md:py-6">
          <div className="flex min-w-0 flex-1 items-center gap-5 md:gap-12">
            ...
            <nav className="ml-auto hidden items-center gap-8 md:flex lg:gap-10" aria-label={h.primaryNavLabel}>
      ...
      {drawerOpen ? (
        <>
          <div
            role="presentation"
            className="fixed inset-0 z-[90] bg-brand-fg/25 backdrop-blur-[2px] md:hidden"
            onClick={closeDrawer}
          />
          <nav
            id="site-mobile-nav"
            className="site-drawer-panel fixed bottom-0 left-0 top-[5.75rem] z-[95] flex w-[min(22rem,88vw)] flex-col overflow-y-auto border-r border-brand-outline-soft/35 bg-brand-bg/96 px-7 py-8 shadow-xl backdrop-blur-xl md:hidden"
            aria-label={h.primaryNavLabel}
          >
